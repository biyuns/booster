...
function Npromotion() {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            ...
            try {
                // 홍보 게시판 API 엔드포인트
                const response = await apiClient.get('/booster/post/intro/category/PROMO');
                response.data.sort((a, b) => new Date(b.create_post_time) - new Date(a.create_post_time));
                setPosts(response.data);
            } catch (error) {
                console.error("홍보 게시판 데이터 로딩 실패:", error);
                setPosts([]);
            } finally {
                setIsLoading(false);
            }
        ...
        <div className="total_ct">
            ...
            <section className="nb-category-ct">
                ...
                <div className="category-promotion">
                    <p style={{color : '#1D1B20'}} onClick={() => navigate('/board/promotion')}> 홍보 </p>
